...
import { getMultiplierFromTime } from '@/lib/game-utils';
...
export function RadarCanvas({ elapsedMs, isCrashed, isFlying }: RadarCanvasProps) {
  ...
  const draw = (ctx: CanvasRenderingContext2D, width: number, height: number) => {
    ...
    const points = 100;
    const duration = isCrashed ? elapsedMs : elapsedMs;
    ctx.moveTo(0, height);
    for (let i = 0; i <= points; i++) {
      const t = (duration / points) * i;
      const m = getMultiplierFromTime(t);
      // Map multiplier and time to canvas coords
      // We scale time to width and multiplier to height
      const x = (t / 15000) * width; // 15s to cross screen
      const y = height - ((m - 1) / 5) * height; // 5x multiplier fills height
      if (x <= width && y >= 0) {
        ctx.lineTo(x, y);
      }
    }
    ...
    ctx.shadowBlur = 0;
    // Draw Plane / Point
    const currentM = getMultiplierFromTime(elapsedMs);
